...
import {
  HYPOTHEK_SIM_STEPS,
  type HypothekScoreResult,
  type HypothekScoreCategory,
} from "@/lib/simulation-hypothek";
import type { OptionKey } from "@/lib/simulation-kontoeröffnung";

interface HypothekResultsScreenProps {
  scores: HypothekScoreResult;
  answers: Record<number, OptionKey>;
  onRetry: () => void;
  onNext: () => void;
}

const CATEGORY_LABELS: Record<HypothekScoreCategory, string> = {
  beratungsqualitaet: "Beratungsqualität",
...
export function HypothekResultsScreen({
  scores,
  answers,
  onRetry,
  onNext,
}: HypothekResultsScreenProps) {
  const [showSolution, setShowSolution] = useState(false);
  const canProceed = scores.overall >= 60;

  const categories: Array<{ key: HypothekScoreCategory; score: number }> = [
    { key: "beratungsqualitaet", score: scores.beratungsqualitaet },
    { key: "fachkompetenz", score: scores.fachkompetenz },
    { key: "kundenorientierung", score: scores.kundenorientierung },
  ];
        ...
        <div className="rounded-DEFAULT bg-surface shadow-card overflow-hidden">
          ...
          {showSolution && (
            <div className="border-t border-border divide-y divide-border">
              {HYPOTHEK_SIM_STEPS.map((step, i) => {
                const chosen = answers[step.id];
                const isCorrect = chosen === step.correctKey;
                const correctOpt = step.options.find((o) => o.key === step.correctKey)!;
