...
import type { HypothekEvaluation } from "./hypothek-antrag-types";

interface HypothekFeedbackCardProps {
  evaluation: HypothekEvaluation;
  isDemo?: boolean;
  onRetry: () => void;
  onBack: () => void;
}

export function HypothekFeedbackCard({
  evaluation,
  isDemo,
  onRetry,
  onBack,
}: HypothekFeedbackCardProps) {
    ...
    <div className="mx-auto max-w-2xl space-y-5 pb-8">
      ...
      >
        <div className="px-6 py-6 text-center">
          ...
          >
            {passed ? "Hypothekarantrag korrekt geprüft" : "Prüfung unvollständig oder fehlerhaft"}
          </h2>
        ...
        {isDemo && (
          ...
          >
            Demo-Auswertung – enthält die vier kritischen Mängel des Brandenberger-Antrags
          </div>
        )}
      </div>
      ...
      <div className="rounded-DEFAULT bg-surface shadow-card p-5">
        <p className="text-sm font-bold text-text-primary mb-2">Empfehlung</p>
        <div className="flex items-center gap-2">
          {evaluation.empfehlungKorrekt ? (
          ...
          <p className="text-sm text-text-secondary">
            {evaluation.empfehlungKorrekt
              ? "Empfehlung korrekt – Ablehnen oder Bewilligen mit Auflagen wäre angemessen"
              : "Empfehlung nicht korrekt – Korrekt wäre: Ablehnen oder Bewilligen mit Auflagen"}
          </p>
        </div>
      </div>
      ...
      >
        ...
        <ul className="space-y-2">
          {[
            "Belehnung max. 80% des Verkehrswerts (FINMA RS 2012/3)",
            "Echte Eigenmittel min. 10% – PK-Vorbezug zählt nicht als echte Eigenmittel",
            "Tragbarkeit max. 33% des Bruttoeinkommens (kalkulatorischer Zins 5%)",
            "2. Hypothek muss innert 15 Jahren auf 65%-Belehnungsgrenze amortisiert werden",
            "Unsicheres Einkommen (befristet, Teilzeit) muss in der Tragbarkeitsberechnung berücksichtigt werden",
          ].map((item, i) => (
        ...
        <div className="mt-4 pt-3 border-t border-border/60">
          <p className="text-xs text-text-secondary font-mono">
            Rechtsgrundlage: FINMA Rundschreiben 2012/3 · Richtlinien SBVg Hypotheken
          </p>
        </div>
      </div>
