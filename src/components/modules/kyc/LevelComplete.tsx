...
import { KYC_LEVELS, type LevelNum, type OptionKey } from "@/lib/kyc";
...
export function LevelComplete({ level, results, onNext, onRetry }: LevelCompleteProps) {
  const levelConfig = KYC_LEVELS.find((l) => l.level === level)!;
              ...
              >
                ...
                {!result?.correct && (
                  <span className="text-xs text-text-secondary">
                    Richtig: {"correct" in scenario ? scenario.correct : "answer" in scenario ? scenario.answer : "—"}
                  </span>
                )}
              </div>
            );
          ...
          {isLastLevel ? (
            <div className="rounded-DEFAULT border border-primary/20 bg-primary-light p-4 text-center">
              ...
              <p className="mt-1 text-xs text-text-secondary">
                Du hast das KYC / Compliance Modul abgeschlossen.
              </p>
            </div>
          ) : (
